...
Feature: Add a Schedule
  ...
  Scenario: Application admin creates schedule via CLI
    ...
    When I run `pawn schedule create "Test 1"`
    Then the schedule named "Test 1" should exist
    And the output should contain "schedule test_1 created"

  Scenario: Application admin creates schedule via CLI
    Given the schedule named "test1" does not exist
    And I have local shell access to execute "bin/pawn" in the project directory
    When I run `pawn schedule create "Test 1" test1`
    Then the schedule named "Test 1" should exist
  ...
  Scenario: Add tournament to schedule
    ...
    When I run `pawn schedule add --to test_schedule jayhs-fall-2013 `
    And I run `pawn schedule show test_schedule`
